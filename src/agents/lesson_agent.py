...
class LessonAgent:
    ...
    def _review_and_polish(self, state: LessonAgentState) -> None:
        ...
        review_prompt = (
            ...
            "4. 如果模板类别是“教学设计”或“综合模版”，要确保内容像教案而不是知识点总结，并尽量结合“配图1/配图2”等进行讲解。\n"
            "5. 如果模板类别是“导学案模板”，要增强任务驱动、题目驱动和学生可完成性。\n"
            "6. 如果模板类别是“综合模版”，要同时保留教师组织感、学生任务感、互动设计和分层训练。\n"
    ...
    def _assess_draft(
        ...
        system_prompt = (
            ...
            "3. 结构是否完整清楚。\n"
            "4. 如果是教学设计或综合模版，是否真正体现图文并茂；如果已有配图却没有结合配图讲解，要严厉扣分。\n"
    ...
    def _build_autonomous_planning_messages(self, state: LessonAgentState) -> List[Message]:
        ...
        if self.request.template_category == "guide":
            ...
            system_prompt += (
                "5. 产出风格必须像综合模版，既有教学目标、教学过程、板书设计，也有学生任务、互动设计、分层巩固与检测。\n"
                "6. 若当前没有可用配图，不要提到配图编号，也不要因为缺图而拒绝生成。\n"
            )
