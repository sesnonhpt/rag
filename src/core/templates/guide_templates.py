...
def build_guide_master_prompt(
    ...
    **kwargs,
    ...
    system_prompt = (
        f"你是一名熟悉一线中小学教学的教研组长，请为主题“{topic}”生成一份可直接发给学生使用的标准导学案。\n"
        ...
        "请严格使用以下结构输出：\n"
        "# 《{topic}》导学案\n"
        "日期：______  班级：______  姓名：______  组别：______  自我评价：______  教师评价：______\n\n"
        "【学习目标】\n"
        "【学习重点、难点】\n"
        "【基础部分】\n"
        "【要点部分】\n"
        ...
        "- 风格要贴近学校真实导学案样例，可按“温故知新、自主探究、分析与解答、读一读、想一想、做一做”等任务语组织。\n"
        "- 【基础部分】偏预习、旧知唤醒、基础梳理；【要点部分】偏核心问题、例题或阅读任务；【拓展部分】偏提升或开放任务；【目标检测】偏当堂练习。\n"
        "- 导学案是学生使用的任务单，不要写成长篇教师讲稿，也不要加入“教学反思”“板书设计”。\n"
        "- 不要输出“参考资源”栏目。\n"
        "- 不要写成纯教师讲稿。\n"
    )
...
def _build_guide_prompt(
    ...
    system_prompt = (
        f"你是一名熟悉一线中小学教学的{subject_label}教师，擅长编写可直接发给学生使用的“导学案”。\n"
        ...
        "- 分条写重点和难点，贴近学校导学案写法。\n\n"
        "【基础部分】\n"
        "- 设计课前预习、教材阅读、温故知新、基础填空、字词/概念/事实梳理等。\n"
        "- 以任务或题目形式表达，保留适量空格或提示。\n"
        "- 风格贴近样例中的“读一读、划一划、分析与解答、自主探究”。\n\n"
        "【要点部分】\n"
        "- 围绕本课核心知识设计典型问题、例题、材料分析、实验探究或语言点训练。\n"
        "- 题目要体现递进关系，并可包含“思考”“小结”“温馨提示”等微栏目。\n"
        "- 要像学生任务单，尽量让学生能直接作答或按提示完成。\n\n"
        "【拓展部分】\n"
